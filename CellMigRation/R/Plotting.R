...
plotAllTracks= function(object,ExpName="ExpName",Type="l") {
  ...
  RangeX=c(MinX,MaxX)
  RangeY=c(MinY,MaxY)
  plot(object[[1]][1:Step,2],object[[1]][1:Step,3],type="o",xlab="X (um)",ylab="Y (um)",col=color[1],las=1,xlim=range(RangeX),ylim=range(RangeY),main=ExpName)
  for(n in 2:Len){
  ...
  plot(object[[1]][1:Step,2],object[[1]][1:Step,3],type="o",xlab="X (um)",ylab="Y (um)",col=color[1],las=1,xlim=range(RangeX),ylim=range(RangeY),main=ExpName)
  for(n in 2:Len){
    points(object[[n]][,2],object[[n]][,3], type='o',col=color[n])
    end<-cbind(object[[n]][Step,2],object[[n]][Step,3])
    points(end,pch=16,col=color[n], cex = 1)
  }
  x=c(min(RangeX)-100,max(RangeX)+100)
...
#' df<-Trajectory_dataset
#' prepro<-PreProcessing(df,PixelSize=1.24, TimeInterval=10)
#' PlotTracksSeparately(prepro, ExpName="Test",Type="b")
#'
PlotTracksSeparately= function(object,ExpName="ExpName",Type="l") {
  if ( ! ( Type %in% c("p","l","b","o") ) ) stop("Type has to be one of the following: p, l, b, o")
  msg <- NULL
  if ( ! is.list(object) ){
    msg <- c(msg, "Input data must be a list. Please run PreProcessing()")
  ...
  for(n in 1:Len){
    ...
    plot(object[[n]][1:Step,2],object[[n]][1:Step,3],type="o",xlab="x (um)",ylab="y (um)",col=color[n],las=1,xlim=range(RangeX),ylim=range(RangeY))
    x=c(min(RangeX)-100,max(RangeX)+100)
    y=c(0,0)
    lines(x, y, type='l', col="black")
    x=c(0,0)
